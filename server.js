...
async function callClaudeWithImages(images, systemPrompt, userPrompt, model, settings = {}) {
    ...
    // claude-opus-4-20250514: 32,000 max
    // claude-sonnet-4-20250514: 32,000 max  
    // claude-haiku-4-5-20251001: 64,000 max
    const maxTokensMap = {
        'claude-opus': 32000,
        'claude-sonnet': 32000,
        'claude-haiku': 64000
    };
    const modelMaxTokens = maxTokensMap[model] || 32000;
...
Format each smartFlag as: { "flag": "description", "category": "improvement|decline|trend|monitoring_gap|critical|medication_interaction", "priority": "info|warning|critical" }`;
        ...
        if (images && images.length > 0) {
            ...
            if (interpreterModel === 'gpt-5.2') {
                ...
                const startTime = Date.now();
                const modelMap = { 'claude-opus': 'claude-opus-4-20250514', 'claude-sonnet': 'claude-sonnet-4-20250514', 'claude-haiku': 'claude-haiku-4-5-20251001' };
                const selectedInterpModel = modelMap[interpreterModel] || modelMap['claude-sonnet'];
                const interpMaxTokensMap = { 'claude-opus': 32000, 'claude-sonnet': 32000, 'claude-haiku': 64000 };
                const interpMaxTokens = interpMaxTokensMap[interpreterModel] || 32000;
                const apiOpts = {
