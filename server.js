        ...
        try {
            if (process.env.DATABASE_URL) {
                ...
                if (roomResult.rows.length > 0) {
                    ...
                    const storedContext = typeof settings.prechartContext === 'string' ? settings.prechartContext.trim() : '';
                    if (storedContext) {
            ...
            console.error(`[${new Date().toISOString()}] Validate prechart context room lookup failed for ${normalizedRoomCode}:`, e.message);
        }
...
`;
    ...
    console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} start (claude) model=${selectedModel} depth=${depth || 'quick'} thinking=${thinkingBudget} suggestions=${includeSuggestions || false} examData=${examData ? 'yes' : 'no'} profile=${userProfile ? 'yes' : 'no'} prechart=${hasPrechartContext ? 'yes' : 'no'} prechartSource=${prechartContextSource} prechartEnabled=${usePrechartContextEnabled ? 'yes' : 'no'} prechartBlend=${prechartBlendModeEnabled ? 'yes' : 'no'} room=${normalizedRoomCode || 'n/a'} reconcileRequested=${reconcileRequested ? 'yes' : 'no'} reconcileApplied=${reconcileApplied ? 'yes' : 'no'} debug=${debugRequested ? 'yes' : 'no'} encounter=${normalizedEncounterType} source=${sourcePage || 'index'} textChars=${text.length} contextChars=${safePrechartContext.length}`);
