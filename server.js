...
app.post('/api/image-analyze/read', async (req, res) => {
    const { images, readerModel = 'claude-sonnet', readerSettings = {}, documentType } = req.body;
    
    if (!images || !images.length) {
        return res.status(400).json({ error: 'No images provided' });
    }
    
    console.log(`[${new Date().toISOString()}] CardioScan read-only: ${images.length} image(s), model=${readerModel}`);
    
    try {
        const systemPrompt = `You are an expert medical document reader. Extract ALL data with perfect accuracy. Return ONLY valid JSON, no markdown.`;
        ...
        if (documentType && DOC_TYPE_PROFILES[documentType]) {
            ...
            const typeList = Object.entries(DOC_TYPE_PROFILES).map(([k, v]) => `"${k}": ${v.label}`).join(', ');
            userPrompt = `Identify this medical document type from: ${typeList}. Then extract ALL data.\n\n${Object.entries(DOC_TYPE_PROFILES).map(([k, p]) => `If ${p.label}:\n${p.extractPrompt}`).join('\n\n')}`;
        }
        ...
        try {
            ...
            extractedData = { rawText: result.text, parseError: true };
        }
        ...
        console.error('CardioScan read error:', err.message);
...
app.post('/api/image-analyze/interpret', async (req, res) => {
    const { extractedData, interpreterModel = 'claude-sonnet', interpreterSettings = {}, images } = req.body;
    
    if (!extractedData) {
        return res.status(400).json({ error: 'No extracted data provided' });
    }
    
    console.log(`[${new Date().toISOString()}] CardioScan interpret-only: model=${interpreterModel}, type=${extractedData.documentType}`);
    
    try {
        const systemPrompt = `You are a board-certified cardiologist providing clinical interpretation. Be specific, evidence-based, actionable. Return ONLY valid JSON.`;
        const detectedType = extractedData.documentType || 'unknown';
        
        const userPrompt = `Interpret this ${DOC_TYPE_PROFILES[detectedType]?.label || 'medical data'}:
...
Return JSON with: clinicalSummary, keyFindings[], abnormalities[], comparisonNotes, suggestedActions[], guidelineAlerts[], differentialConsiderations[], billingCodes[], smartFlags[]`;
        ...
        if (images && images.length > 0) {
            ...
            // Text-only interpretation (no images needed)
        ...
        console.error('CardioScan interpret error:', err.message);
