...
async function initDB() {
    try {
        ...
        // ── Dictation history — persists every Send-to-PC ──
        ...
        await pool.query(`
            CREATE TABLE IF NOT EXISTS dictations (
                ...
                text TEXT NOT NULL,
                preview VARCHAR(400) DEFAULT '',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        `);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_dictations_room ON dictations(room)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_dictations_room_date ON dictations(room, created_at DESC)`);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_dictations_text_trgm ON dictations USING GIN (text gin_trgm_ops)`);
...
// ── Cheap AI patient name extraction (GPT-4o-mini, async/fire-and-forget) ──
async function extractPatientName(dictationId, text) {
    try {
        if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY.startsWith('sk-fake')) return;
        ...
        const resp = await openai.chat.completions.create({
            ...
            messages: [{
                role: 'user',
                content: `Extract ONLY the patient's name from this medical dictation snippet. If no patient name is found, respond with exactly: NONE\n\nReturn ONLY the name (e.g. "John Smith") or "NONE". No explanation.\n\n"${first100Words}"`
            }]
        });
        const name = resp.choices[0]?.message?.content?.trim();
        if (name && name !== 'NONE' && name.length < 200) {
            await pool.query('UPDATE dictations SET patient_name = $1 WHERE id = $2', [name, dictationId]);
            console.log(`[AI] Dictation #${dictationId} tagged: ${name}`);
        }
    } catch (err) {
        console.log(`[AI] Name extraction failed for #${dictationId}: ${err.message}`);
        // Silent fail — dictation is already saved, name is just a bonus
    }
}
...
const prechartContextMemory = new Map();

// In-memory fallback for templates (if no DB)
const templatesMemory = new Map();
const favoritesMemory = new Map();

const SAFE_VALIDATE_PROMPT_BLOCK_META = {
    coreGuardrailsGpt: {
        label: 'Core Guardrails (GPT-5.2)',
        description: 'The main editing/rewriting guardrails for GPT validation.'
    },
    coreGuardrailsClaude: {
        label: 'Core Guardrails (Claude)',
        description: 'The main validation/editing guardrails for Claude validation.'
    },
    advancedReasoning: {
        label: 'Advanced Reasoning Style',
        description: 'The advanced cardiology/NEJM-style reasoning block.'
    },
    apVoiceStyle: {
        label: 'A&P Voice Style',
        description: 'The cardiology attending-style Assessment and Plan voice used during rewrite.'
    }
};

const SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS = {
    coreGuardrailsGpt: `You are a meticulous medical documentation editor with deep clinical reasoning ability.

Rewrite and format the provided clinician draft into a clean, professional note. Preserve ALL clinical meaning. Do NOT add new facts, diagnoses, meds, vitals, or test results that are not already present or clearly implied in the original. Fix grammar, spelling, terminology, and structure. Use your clinical understanding to interpret the clinician's intent and express it more precisely - e.g., if the clinician describes a pattern consistent with a known entity, use the correct terminology even if they stated it loosely. Resolve ambiguous phrasing by rewording clearly - do not infer new clinical facts. Do NOT include ICD-10 codes unless they were explicitly dictated in the original note. Keep patient identifiers out. Keep concise but complete.

Never comment on what is missing or absent from the dictation. If a section was not dictated, omit it silently. Do not insert phrases like "not documented", "not provided", "per dictation", "not included", "deferred", or any language that calls attention to omitted content. The output should read as if the clinician wrote it directly.`,
    coreGuardrailsClaude: `You are an expert medical transcription reviewer and clinical documentation editor with deep clinical reasoning ability. Analyze this dictation thoroughly. Use your clinical understanding to interpret the clinician's intent and identify errors, contradictions, and issues. Never comment on what is missing or absent from the dictation - focus only on what IS documented. Do not insert phrases like "not documented", "not provided", "per dictation", "not included", or "deferred".`,
    advancedReasoning: `
## ADVANCED CLINICAL REASONING STYLE:
Show very advanced logic and NEJM-style case-discussion reasoning in Analysis and Assessment/Plan, at attending-level subspecialty depth like a world-expert cardiologist, drawing from ALL available data.
- Synthesize ALL available data (history, exam, diagnostics, meds, trends, prior context) into one coherent clinical narrative.
- Explicitly connect data points to each diagnostic and therapeutic decision.
- Prioritize differential diagnoses by likelihood and risk, and explain why.
- Cover both common and less common etiologies when needed; common causes first, rare causes only when supported by specific red flags or discordant findings.
- Use evidence-based, guideline- and landmark-trial-informed reasoning without fabricating citations.
- Avoid generic filler; keep statements specific, patient-linked, and decision-relevant.
- Avoid duplicated thoughts/plan items across sections; merge overlap into one clean plan.
- If data conflict, reconcile the conflict explicitly and state what most likely explains it.
`,
    apVoiceStyle: `
## CLINICAL VOICE - ASSESSMENT & PLAN:
The author is a board-certified cardiologist with advanced clinical reasoning. The Assessment & Plan must reflect that level of expertise:
- Write the A&P discussion in the style of an NEJM clinical case discussion - thoughtful, precise, demonstrating pathophysiologic reasoning and differential consideration
- For each problem, briefly articulate the clinical reasoning: WHY this diagnosis, WHAT data supports it, WHAT the key decision points are
- Address all relevant cardiac domains within the data: ischemic heart disease, structural/LV systolic and diastolic function, valvular disease, pericardial disease, hypertension, RV function, arrhythmia/EP, and lipids
- Use evidence-based language and reference guideline-concordant management where applicable (e.g., "consistent with ACC/AHA Stage C HFrEF, warranting initiation of four-pillar GDMT")
- Do NOT invent findings or diagnoses not supported by the dictation - but DO connect the dots between documented findings with expert-level reasoning
- The tone should read as if authored by a subspecialty attending presenting at a cardiology case conference - confident, precise, and clinically sophisticated
`
};

function sanitizeSafeValidatePromptOverrides(rawOverrides) {
    const clean = {};
    const input = rawOverrides && typeof rawOverrides === 'object' ? rawOverrides : {};

    Object.keys(SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS).forEach((key) => {
        if (typeof input[key] !== 'string') return;
        const normalized = input[key].replace(/\r\n/g, '\n').trim();
        if (!normalized) return;
        if (normalized === SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS[key].trim()) return;
...
function getEffectiveSafeValidatePromptBlocks(rawOverrides) {
    const overrides = sanitizeSafeValidatePromptOverrides(rawOverrides);
    const blocks = {};

    Object.keys(SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS).forEach((key) => {
        blocks[key] = overrides[key] || SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS[key];
    });

    return {
        meta: SAFE_VALIDATE_PROMPT_BLOCK_META,
        defaults: SAFE_VALIDATE_PROMPT_BLOCK_DEFAULTS,
        overrides,
        blocks
    };
}
...
setInterval(() => {
    ...
    for (const [room, data] of prechartContextMemory.entries()) {
        if (now - data.timestamp > 86400000) {
            prechartContextMemory.delete(room);
        }
    }
}, 300000);
...
app.post('/api/send', (req, res) => {
    ...
    // ── NEW: Persist to Postgres (async, doesn't affect response) ──
    if (process.env.DATABASE_URL) {
        const preview = text.split(/\s+/).slice(0, 50).join(' ');
        pool.query(
            'INSERT INTO dictations (room, text, preview, created_at) VALUES ($1, $2, $3, NOW()) RETURNING id',
            [room, text, preview]
        ).then(result => {
            const id = result.rows[0].id;
            console.log(`[${new Date().toISOString()}] Dictation #${id} saved for room ${room}`);
            // Fire-and-forget AI name extraction
            extractPatientName(id, text);
        }).catch(err => {
            console.error('Dictation save error:', err.message);
        });
    }
});
...
// ============ SAFE VALIDATE PROMPT BLOCKS API ============

app.get('/api/validate-prompt-blocks/:code', async (req, res) => {
    const normalizedCode = typeof req.params.code === 'string' ? req.params.code.trim().toUpperCase() : '';

    try {
        if (!normalizedCode || !process.env.DATABASE_URL) {
            return res.json({
                ...getEffectiveSafeValidatePromptBlocks({}),
                updatedAt: null
            });
        }
        ...
        return res.json({
            ...getEffectiveSafeValidatePromptBlocks(settings.validatePromptBlocks || {}),
            updatedAt: settings.validatePromptBlocksUpdatedAt || null
        });
    } catch (e) {
        console.error('Error loading validate prompt blocks:', e.message);
        res.json({
            ...getEffectiveSafeValidatePromptBlocks({}),
            updatedAt: null
        });
...
app.put('/api/validate-prompt-blocks/:code', async (req, res) => {
    const normalizedCode = typeof req.params.code === 'string' ? req.params.code.trim().toUpperCase() : '';
    const { blocks } = req.body || {};
    const overrides = sanitizeSafeValidatePromptOverrides(blocks);
        ...
        await pool.query(
            `INSERT INTO room_settings (storage_code, settings, updated_at)
             VALUES (
                $1,
                jsonb_build_object(
                    'validatePromptBlocks', $2::jsonb,
                    'validatePromptBlocksUpdatedAt', $3::text
                ),
                CURRENT_TIMESTAMP
             )
             ON CONFLICT (storage_code)
             DO UPDATE SET settings = jsonb_set(
                    jsonb_set(
                        COALESCE(room_settings.settings, '{}'::jsonb),
                        '{validatePromptBlocks}',
                        $2::jsonb,
                        true
                    ),
                    '{validatePromptBlocksUpdatedAt}',
                    to_jsonb($3::text),
                    true
                ),
                updated_at = CURRENT_TIMESTAMP`,
            [normalizedCode, JSON.stringify(overrides), updatedAtIso]
        );

        console.log(`[${new Date().toISOString()}] Safe validate prompt blocks updated for ${normalizedCode}: ${Object.keys(overrides).join(', ') || 'defaults only'}`);
        res.json({
            success: true,
            ...getEffectiveSafeValidatePromptBlocks(overrides),
            updatedAt: updatedAtIso
        });
    } catch (e) {
        console.error('Error saving validate prompt blocks:', e.message);
        res.status(500).json({ error: e.message });
    }
});

// ============ PRECHART CONTEXT API ============

// Get latest PreChart context for a room
app.get('/api/prechart-context/:code', async (req, res) => {
    const { code } = req.params;
    ...
    try {
        if (process.env.DATABASE_URL) {
            ...
            if (result.rows.length > 0) {
                ...
                const context = settings.prechartContext || '';
                return res.json({
                    context,
                    updatedAt: settings.prechartContextUpdatedAt || null,
                    source: settings.prechartContextSource || null,
                    patientName: settings.prechartContextPatientName || null,
                    patientIndex: settings.prechartContextPatientIndex ?? null,
                    sessionId: settings.prechartContextSessionId || null
                });
            }
        }
        
        const mem = prechartContextMemory.get(code);
        if (mem) {
            return res.json({
                context: mem.context || '',
                updatedAt: mem.updatedAt || null,
                source: mem.source || null,
                patientName: mem.patientName || null,
                patientIndex: mem.patientIndex ?? null,
                sessionId: mem.sessionId || null
            });
        }
        
        res.json({ context: '', updatedAt: null, source: null, patientName: null, patientIndex: null, sessionId: null });
    } catch (e) {
        console.error('Error loading prechart context:', e.message);
        const mem = prechartContextMemory.get(code);
        res.json({
            context: mem?.context || '',
            updatedAt: mem?.updatedAt || null,
...
// Set/replace PreChart context for a room
app.post('/api/prechart-context/:code', async (req, res) => {
    const { code } = req.params;
    const {
        context = '',
        source = 'prechart-ios',
    ...
    const trimmedContext = context.trim().slice(0, 60000);
    const updatedAtIso = new Date().toISOString();
    
    // Always keep in-memory fallback current
    prechartContextMemory.set(code, {
            ...
            await pool.query(
                `INSERT INTO room_settings (storage_code, settings, updated_at)
                 VALUES (
                    $1,
                    jsonb_build_object(
                        'prechartContext', $2::text,
                        'prechartContextUpdatedAt', $3::text,
                        'prechartContextSource', $4::text,
                        'prechartContextPatientName', $5::text,
                        'prechartContextPatientIndex', to_jsonb($6::int),
                        'prechartContextSessionId', $7::text
                    ),
                    CURRENT_TIMESTAMP
                 )
                 ON CONFLICT (storage_code)
                 DO UPDATE SET settings = room_settings.settings || jsonb_build_object(
                        'prechartContext', $2::text,
                        'prechartContextUpdatedAt', $3::text,
                        'prechartContextSource', $4::text,
                        'prechartContextPatientName', $5::text,
                    ...
                    updated_at = CURRENT_TIMESTAMP`,
                [code, trimmedContext, updatedAtIso, source, patientName, patientIndex, sessionId]
            );
        }
        
        console.log(`[${new Date().toISOString()}] PreChart context saved for ${code}: ${trimmedContext.length} chars`);
        res.json({ success: true, room: code, chars: trimmedContext.length, updatedAt: updatedAtIso });
    } catch (e) {
        console.error('Error saving prechart context:', e.message);
        res.status(500).json({ error: e.message });
    }
});

// ============ PROFILE API (persisted in room_settings JSONB) ============

// Get profile for a room
app.get('/api/profile/:code', async (req, res) => {
    ...
    try {
        if (process.env.DATABASE_URL) {
            ...
            const profile = result.rows.length > 0 ? (result.rows[0].settings?.profile || '') : '';
            res.json({ profile });
        } else {
            res.json({ profile: '' });
        }
...
// Save profile for a room (merges into existing settings JSONB)
app.put('/api/profile/:code', async (req, res) => {
    const { code } = req.params;
    const { profile } = req.body;
    try {
        if (process.env.DATABASE_URL) {
            // Upsert: merge profile into the existing settings JSON
            await pool.query(
                `INSERT INTO room_settings (storage_code, settings, updated_at)
                 VALUES ($1, jsonb_build_object('profile', $2::text), CURRENT_TIMESTAMP)
                 ON CONFLICT (storage_code)
                 DO UPDATE SET settings = room_settings.settings || jsonb_build_object('profile', $2::text),
                              updated_at = CURRENT_TIMESTAMP`,
                [code, profile || '']
            );
            console.log(`[${new Date().toISOString()}] Profile saved for room ${code} (${(profile || '').length} chars)`);
            res.json({ success: true });
        } else {
            res.status(400).json({ error: 'Database not configured' });
        }
...
app.post('/api/old-note-digest', async (req, res) => {
    const {
        text,
        digestPrompt,
        model,
    ...
    if (!text || !String(text).trim()) {
        return res.status(400).json({ error: 'Old note text is required' });
    }

    const normalizedRoomCode = typeof roomCode === 'string' ? roomCode.trim().toUpperCase() : '';
    const requestedUserProfile = typeof userProfile === 'string' ? userProfile.trim() : '';
    let roomSettings = null;

    if (normalizedRoomCode && process.env.DATABASE_URL) {
        try {
            ...
            if (roomResult.rows.length > 0) {
                roomSettings = roomResult.rows[0].settings || {};
            }
        } catch (e) {
            console.error(`[${new Date().toISOString()}] Old-note digest room settings lookup failed for ${normalizedRoomCode}:`, e.message);
        }
    }

    const storedRoomProfile = typeof roomSettings?.profile === 'string' ? roomSettings.profile.trim() : '';
    const storedRoomDigestPrompt = typeof roomSettings?.prechart2OldNoteDigestPrompt === 'string'
        ? roomSettings.prechart2OldNoteDigestPrompt.trim()
        : '';
    const requestedDigestPrompt = typeof digestPrompt === 'string' ? digestPrompt.trim() : '';
    const effectiveUserProfile = storedRoomProfile || requestedUserProfile;
    const effectiveDigestPrompt = storedRoomDigestPrompt || requestedDigestPrompt || `Extract a high-yield physician-facing bullet digest from this older note.

Focus especially on:
- prior issues / major cardiovascular and relevant non-cardiovascular problems
- prior tests and important prior results, with dates if explicitly stated
- last known LVEF or other key ventricular function data
- stress / ischemia / cath / coronary disease history
- heart failure, valve, EP/rhythm, pericardial, diastolic HF, amyloid, pulmonary HTN, congenital, and other major issues if present
- prior procedures and interventions
- prior suggested plan, anticipated plan, or decision logic
- what should not be forgotten when seeing the patient again

Output plain text only under these headings:
PRIOR KEY HISTORY
PRIOR TESTS / RESULTS
PRIOR CLINICAL THINKING
PRIOR / ANTICIPATED PLAN
COPY-FORWARD PEARLS

Hard rules:
- use only what is present in the note
- do not invent dates, diagnoses, tests, or plans
- do not rewrite this as today's note
- keep it clinically dense, concise, and review-friendly`;
    const sourceText = String(text).trim().slice(0, 60000);
    const selectedModel = model === 'opus' ? 'opus' : (model === 'gpt-5.2' ? 'gpt-5.2' : 'sonnet');
    const selectedDepth = typeof depth === 'string' ? depth : 'medium';

    let systemPrompt = `You are summarizing an older physician note for fast expert review before a current encounter.
Use the user-configured extraction prompt below as the main instruction set for what to pull out.

Configured digest prompt:
${effectiveDigestPrompt}

Global hard rules:
- Use only facts present in the source note.
- Do NOT invent new diagnoses, tests, or management.
- Do NOT rewrite the note as today's note.
- Do NOT present speculative old ideas as current truth.
- Keep it concise, high-yield, and clinically serious.
- Omit patient identifiers when possible.
- Output plain text only.`;

    if (effectiveUserProfile) {
        systemPrompt += `\n\nUser profile preferences for tone and emphasis:\n${effectiveUserProfile}`;
    }

    const userPrompt = `Create the digest from this older note:\n\n${sourceText}`;
    ...
    try {
        if (selectedModel === 'gpt-5.2') {
            ...
            const reasoningEffort = reasoningEffortMap[selectedDepth] || 'medium';
            const apiCallParams = {
                model: 'gpt-5.2',
                input: [
                    { role: 'system', content: systemPrompt },
            ...
            if (reasoningEffort === 'none') {
                ...
                apiCallParams.max_output_tokens = 16000;
            }

            const response = await openai.responses.create(apiCallParams);
            const digest = (response.output_text || '').trim();
            ...
            return res.json({
                ...
                model: 'gpt-5.2',
                modelLabel: 'GPT-5.2',
                promptSource: storedRoomDigestPrompt ? 'server' : (requestedDigestPrompt ? 'request' : 'default')
            });
        }
        ...
        const pricingKey = selectedModel === 'opus' ? 'claude-opus' : 'claude-sonnet';
        const anthropicModel = modelMap[selectedModel] || modelMap.sonnet;
        ...
        const thinkingBudget = thinkingBudgetMap[selectedDepth] || 5000;
        const maxTokens = selectedModel === 'opus' ? 24000 : 16000;
        const apiOptions = {
        ...
        if (thinkingBudget > 0) {
            apiOptions.thinking = { type: 'enabled', budget_tokens: Math.min(thinkingBudget, maxTokens - 2048) };
        }

        const stream = anthropic.messages.stream(apiOptions);
        const finalMessage = await stream.finalMessage();
        const digest = (finalMessage.content || [])
        ...
        return res.json({
            ...
            digest,
            model: anthropicModel,
            modelLabel: COST_TABLE[pricingKey]?.name || anthropicModel,
            promptSource: storedRoomDigestPrompt ? 'server' : (requestedDigestPrompt ? 'request' : 'default')
        });
    } catch (error) {
        console.error(`[${new Date().toISOString()}] Old-note digest error:`, error.message);
        res.status(500).json({ error: error.message });
    }
});

app.post('/api/test-result-digest', async (req, res) => {
    const {
        text,
        digestPrompt,
        model,
    ...
    if (!text || !String(text).trim()) {
        return res.status(400).json({ error: 'Test or procedure text is required' });
    }
    ...
    if (normalizedRoomCode && process.env.DATABASE_URL) {
        try {
            ...
            console.error(`[${new Date().toISOString()}] Test-result digest room settings lookup failed for ${normalizedRoomCode}:`, e.message);
        }
    }

    const storedRoomProfile = typeof roomSettings?.profile === 'string' ? roomSettings.profile.trim() : '';
    const storedRoomDigestPrompt = typeof roomSettings?.prechart2TestResultDigestPrompt === 'string'
        ? roomSettings.prechart2TestResultDigestPrompt.trim()
        : '';
    const requestedDigestPrompt = typeof digestPrompt === 'string' ? digestPrompt.trim() : '';
    const effectiveUserProfile = storedRoomProfile || requestedUserProfile;
    const effectiveDigestPrompt = storedRoomDigestPrompt || requestedDigestPrompt || `Digest this procedure note or test result for a physician who wants the key takeaways fast.

Focus especially on:
- what the test or procedure shows
- any explicit conclusion, impression, final interpretation, or summary stated in the report
- key quantitative findings, dates, comparisons, and changes from prior if explicitly stated
- what diagnoses or pathophysiology the result supports or weakens
- any conflicts, internal inconsistencies, or statements that do not fit the actual findings
- whether the interpretation/logic seems too strong, too weak, or incomplete based on the text alone
- what would matter most to carry forward into today's clinical thinking

Output plain text only under these headings:
TEST / PROCEDURE SUMMARY
REPORT CONCLUSION / IMPRESSION
KEY FINDINGS
CONFLICTS / CONCERNS
LOGIC CHECK / IF I DISAGREE
COPY-FORWARD PEARLS

Hard rules:
- use only what is present in the source text
- do not invent diagnoses, measurements, dates, or criticisms
- if there is no conflict or disagreement, say that clearly and briefly
- keep the digest clinically dense, concise, and decision-relevant`;
    const sourceText = String(text).trim().slice(0, 60000);
    const selectedModel = model === 'opus' ? 'opus' : (model === 'gpt-5.2' ? 'gpt-5.2' : 'sonnet');
    const selectedDepth = typeof depth === 'string' ? depth : 'medium';

    let systemPrompt = `You are digesting a medical test result, procedure report, or interpretation note for a physician who wants the key takeaways quickly.
Use the user-configured extraction prompt below as the main instruction set for what to pull out.
...
- Do NOT invent new diagnoses, tests, or management.
- Do NOT rewrite this as today's note.
- If the source contains a labeled or stated conclusion/impression/final interpretation, extract it explicitly and prominently.
- Keep the digest concise, clinically serious, and decision-relevant.
- Output plain text only.`;
    ...
    const userPrompt = `Create the digest from this test/procedure note:\n\n${sourceText}`;
    ...
    try {
        ...
        console.error(`[${new Date().toISOString()}] Test-result digest error:`, error.message);
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/validate', async (req, res) => {
    const { 
        ...
        prechartBlendMode,
        currentTruthText,
        currentTruthTimestamp,
        roomCode,
        encounterType,
        sourcePage,
        reconcileMode,
        prechartDebug,
        stream: clientWantsSSE
    } = req.body;
    ...
    const normalizedRoomCode = typeof roomCode === 'string' ? roomCode.trim().toUpperCase() : '';
    const requestedUserProfile = typeof userProfile === 'string' ? userProfile.trim() : '';
    const normalizedEncounterType = encounterType === 'new_consult' ? 'new_consult' : 'follow_up';
    const isPrechartPage = sourcePage === 'prechart' || sourcePage === 'prechart2';
    const usePrechartContextEnabled = usePrechartContext === undefined ? true : isTruthy(usePrechartContext);
    const prechartBlendModeEnabled = isTruthy(prechartBlendMode);
    const reconcileRequested = isTruthy(reconcileMode);
    let roomSettings = null;

    // Canonical profile/context source: room settings on server (by room code), when available.
    if (normalizedRoomCode && process.env.DATABASE_URL) {
        try {
            const roomResult = await pool.query(
                'SELECT settings FROM room_settings WHERE storage_code = $1',
            ...
            console.error(`[${new Date().toISOString()}] Validate room settings lookup failed for ${normalizedRoomCode}:`, e.message);
        }
    }

    const storedRoomProfile = typeof roomSettings?.profile === 'string' ? roomSettings.profile.trim() : '';
    let effectiveUserProfile = requestedUserProfile;
    let profileSource = effectiveUserProfile ? 'request' : 'none';
    if (storedRoomProfile) {
        if (requestedUserProfile && requestedUserProfile !== storedRoomProfile) {
            console.log(`[${new Date().toISOString()}] Validate profile override: using room_settings profile for ${normalizedRoomCode} instead of request payload`);
        }
        effectiveUserProfile = storedRoomProfile;
        profileSource = 'room_settings';
    }
    
    let resolvedPrechartContext = typeof prechartContext === 'string' ? prechartContext.trim() : '';
    let prechartContextSource = resolvedPrechartContext ? 'request' : 'none';
    
    if (!usePrechartContextEnabled) {
        resolvedPrechartContext = '';
        prechartContextSource = 'disabled_by_client';
    }
    
    // Server-side safety net: if PreChart page requested context mode but payload context is empty,
    // resolve from room settings/memory so Validate still gets the baseline.
    if (!resolvedPrechartContext && isPrechartPage && usePrechartContextEnabled && normalizedRoomCode) {
        const storedContext = typeof roomSettings?.prechartContext === 'string' ? roomSettings.prechartContext.trim() : '';
        if (storedContext) {
            resolvedPrechartContext = storedContext;
            prechartContextSource = 'room_settings';
        }
        
        if (!resolvedPrechartContext) {
            const mem = prechartContextMemory.get(normalizedRoomCode);
            const memContext = typeof mem?.context === 'string' ? mem.context.trim() : '';
            if (memContext) {
                resolvedPrechartContext = memContext;
                prechartContextSource = 'memory_fallback';
            }
        }
    }
    
    const hasPrechartContext = resolvedPrechartContext.length > 0;
    const safePrechartContext = hasPrechartContext ? resolvedPrechartContext.slice(0, 30000) : '';
    const requestedCurrentTruthText = typeof currentTruthText === 'string' ? currentTruthText.trim() : '';
    const hasCurrentTruthAnchor = sourcePage === 'prechart2' && requestedCurrentTruthText.length > 0;
    const safeCurrentTruthText = hasCurrentTruthAnchor ? requestedCurrentTruthText.slice(0, 20000) : '';
    const reconcileApplied = hasPrechartContext && (reconcileRequested || isPrechartPage);
    const debugRequested = isTruthy(prechartDebug);
    const validateRunId = `v_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const safePromptBlocks = getEffectiveSafeValidatePromptBlocks(roomSettings?.validatePromptBlocks || {}).blocks;
    const coreGuardrailsGpt = safePromptBlocks.coreGuardrailsGpt;
    const coreGuardrailsClaude = safePromptBlocks.coreGuardrailsClaude;
    const advancedReasoningInstruction = `\n${safePromptBlocks.advancedReasoning.trim()}\n`;
    const apVoiceStyleInstruction = `\n${safePromptBlocks.apVoiceStyle.trim()}\n`;
    
    console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} source=${sourcePage || 'index'} model=${model || 'sonnet'} profile=${effectiveUserProfile ? 'yes' : 'no'} profileSource=${profileSource} prechartContext=${hasPrechartContext ? 'yes' : 'no'} prechartSource=${prechartContextSource} prechartEnabled=${usePrechartContextEnabled ? 'yes' : 'no'} prechartBlend=${prechartBlendModeEnabled ? 'yes' : 'no'} currentTruth=${hasCurrentTruthAnchor ? 'yes' : 'no'} currentTruthTs=${currentTruthTimestamp || 0} room=${normalizedRoomCode || 'n/a'} reconcileRequested=${reconcileRequested ? 'yes' : 'no'} reconcileApplied=${reconcileApplied ? 'yes' : 'no'} encounter=${normalizedEncounterType} textChars=${text.length} contextChars=${safePrechartContext.length}`);
    const prechartBlendModeInstruction = prechartBlendModeEnabled
        ? `
## PRECHART BLEND MODE:
The note may contain a PreChart prior-data/thought-process segment followed by today's dictation.
- Treat the PreChart segment as prior history and prior thought-process context.
- Integrate relevant prior thoughts into today's reasoning progression when still clinically appropriate.
- Do NOT duplicate overlapping assessment/plan ideas; merge repeated thoughts once.
- Today's dictation remains authoritative for final current decisions and plan.
`
        : '';
    const prechart2DigestBlockInstruction = sourcePage === 'prechart2'
        ? `
## PRECHART2 SUPPLEMENTAL DIGEST BLOCKS:
The working note may contain labeled supplemental blocks such as:
- OLD NOTE DIGEST (background only)
- TEST / PROCEDURE DIGEST (background only)
- COPY-FORWARD PEARLS FROM OLD NOTE
- COPY-FORWARD PEARLS FROM TEST / PROCEDURE DIGEST

Treat these labeled digest blocks as supplemental historical/background context, NOT as today's current encounter truth.
- Today's fresh dictation/current encounter text remains authoritative for current status, assessment, and plan.
- You may use the digest blocks to avoid missing prior issues, prior tests, prior reasoning, or important carry-forward pearls.
- Do NOT let a digest block override fresh current dictation.
- If a digest block conflicts with today's dictation, today's dictation wins.
`
        : '';
    const prechart2CurrentTruthInstruction = hasCurrentTruthAnchor
        ? `
## PRECHART2 CURRENT TRUTH ANCHOR:
The block below is the most recent live dictation/current encounter send and is the authoritative current truth.
- Treat this anchor as the most recent clinical update.
- If any other content in the working note conflicts with it, this anchor wins.
- Use PreChart context and labeled digest blocks only to supplement this anchor, not override it.

CURRENT TRUTH ANCHOR:
${safeCurrentTruthText}
`
        : '';
    // ============ GPT-5.2 PATH ============
    ...
    if (model === 'gpt-5.2') {
        ...
        try {
            ...
            // Generic medical documentation editor — specialty flavor comes from room profile
            let systemPrompt = coreGuardrailsGpt;
            
            if (hasPrechartContext) {
                systemPrompt += reconcileApplied
                    ? `

## PRECHART RECONCILIATION MODE:
You are reconciling two sources into ONE current encounter note:
1) PreChart baseline (pre-visit background)
2) Today's dictation update (authoritative current encounter)

Hard merge rules:
- Today's dictation is the source of truth for current status, assessment, and plan.
- If baseline and today's dictation conflict, today's dictation wins.
- Do NOT carry forward baseline plan/orders unless explicitly restated today.
- Baseline plan content is historical prep context only.
- Carry forward stable background facts from baseline when not contradicted today.
- Perform a deliberate baseline history sweep: chronic diagnoses, prior procedures, allergies/intolerances, key prior test trends, and major social/family risk factors.
- Do NOT omit clinically relevant prior history from baseline just because today's dictation is brief.
- If baseline history is not restated today but remains relevant and not contradicted, keep it as historical context (e.g., "History of ..."), not as a new active plan item.
- Clearly prioritize interval changes and "today vs prior" framing.
- Encounter mode is ${normalizedEncounterType}.
`
                    : `

## PRECHART CONTEXT MODE:
You are receiving two inputs:
1) PreChart context (background and pre-visit thoughts)
2) Current dictation (today's encounter and authoritative source of truth)

Hard rules:
- Current dictation ALWAYS wins for today's status, assessment, and plan.
- If PreChart context conflicts with today's dictation, follow today's dictation.
- Do NOT carry forward plan/orders from PreChart unless explicitly re-stated in today's dictation.
- Treat PreChart plan statements as historical pre-visit thoughts only.
- Use PreChart only for background and prior-trend framing.
- Perform a baseline history sweep and preserve clinically relevant prior history unless contradicted by today's dictation.
- If a baseline history item is not restated today, keep it concise as historical context rather than dropping it.
- Encounter mode is ${normalizedEncounterType}.`;
            }
            if (prechartBlendModeEnabled) {
                systemPrompt += prechartBlendModeInstruction;
            }
            systemPrompt += prechart2DigestBlockInstruction;
            systemPrompt += prechart2CurrentTruthInstruction;
            systemPrompt += advancedReasoningInstruction;

            // Add clinical reasoning partner when suggestions are enabled
            if (includeSuggestions) {
                systemPrompt += `

## CLINICAL REASONING PARTNER:
Additionally, act as a clinical reasoning partner: consider the full differential, identify findings discordant with the stated assessment, flag when the clinical picture suggests something the note may be missing, and evaluate whether the workup is appropriate and complete.`;
            }
...
Return ONLY valid JSON (no markdown, no backticks) with this exact structure:
{
  ...
  ],` : ''}${apGapCheck && rewriteFullNote ? `
  "apGaps": [
    {
      "finding": "The specific clinical finding from HPI, exam, or diagnostics not addressed in A&P",
      "source": "HPI|Physical Exam|Diagnostics|ROS",
      "planItem": "A complete, ready-to-insert A&P entry: problem name, brief clinical reasoning tied to this patient's specific data, and concrete evidence-based management plan with specific tests, medications, or interventions"
    }
  ],` : ''}
  "summary": "Brief 1-line summary"
}

If no issues found: {"issues": [], "icd10": [], "guidelineAlerts": [], "suggestions": null, ${rewriteFullNote ? '"rewrittenNote": null, "rewrittenAP": null, "originalAPStart": null, "originalAPEnd": null, "removedContent": [], ' : ''}${apGapCheck ? '"apGaps": [], ' : ''}"summary": "No issues found"}

## ICD-10 CODES — DO NOT GENERATE:
Do NOT generate or include ICD-10 codes. The "icd10" array must ALWAYS be empty []. Do not append ICD-10 codes to the rewritten note. The clinician will add their own ICD codes separately.`;
...
- Do NOT add ICD-10 codes to the rewritten note unless they were explicitly present in the original dictation

${apVoiceStyleInstruction}
...
`;
            ...
            // Add A&P Gap Detection if toggle is on AND rewrite is on
            if (apGapCheck && rewriteFullNote) {
                systemPrompt += `

## A&P GAP DETECTION:
Compare the ORIGINAL dictated Assessment & Plan against the HPI, physical exam, ROS, and diagnostics. Identify clinical findings documented elsewhere in the note that are NOT explicitly addressed as a problem or plan item in the ORIGINAL A&P. Be thorough — err on the side of flagging rather than missing.

Examples of gaps to flag:
- An abnormal lab value mentioned in diagnostics but no A&P item addressing it
- A symptom in HPI (e.g., syncope, edema, palpitations) without a corresponding A&P problem
- An abnormal exam finding (e.g., new murmur, elevated JVP, rales) not addressed in plan
- A chronic condition mentioned in HPI but missing from the problem list
- A medication mentioned without a corresponding diagnosis in A&P
- Risk factors documented but not addressed (e.g., smoking, obesity, family history of SCD)

For each gap, generate a complete "planItem" that:
- Names the problem specifically
- References this patient's actual data from the note (not generic statements)
- Includes a concrete, evidence-based management plan with specific tests, medications, interventions, or follow-up
- Is written at the clinical level appropriate for the clinician's profile
- Is ready to paste directly into the A&P section with no editing needed

IMPORTANT: Compare against the ORIGINAL dictated A&P, not your rewritten version. Even if your rewrittenNote addresses something, if the original dictation's A&P did not, flag it as a gap.

CRITICAL: Do NOT include apGaps content in the "rewrittenNote" or "rewrittenAP" fields. The rewritten note must only contain what the clinician actually dictated (cleaned up). The apGaps are SEPARATE suggestions that the clinician will choose to add manually. Keep them strictly in the "apGaps" array only.
`;
            }

            // Build user prompt
            const userPrompt = hasPrechartContext
                ? reconcileApplied
                    ? `Please reconcile baseline context with today's dictation${rewriteFullNote ? ', then provide both a complete rewritten version AND a separate A&P-only rewrite' : ''}.

This is a merge task, not baseline-only validation.
Use today's dictation as authoritative for current status and plan.
Do not carry baseline plan items unless they are explicitly restated today.
Do not miss clinically relevant prior history from baseline; retain it as historical context when not contradicted.

<encounter_type>
${normalizedEncounterType}
</encounter_type>

<prechart_baseline_background>
${safePrechartContext}
</prechart_baseline_background>

<today_dictation_update_authoritative>
${text}
</today_dictation_update_authoritative>`
                    : `Please review this dictated medical note and identify any issues${rewriteFullNote ? ', then provide both a complete rewritten version AND a separate A&P-only rewrite' : ''}.

Use current dictation as primary truth for today's note, and use PreChart context as background only.
Do a deliberate baseline history sweep so relevant prior history is not accidentally dropped.

<encounter_type>
${normalizedEncounterType}
</encounter_type>

<prechart_context_background>
${safePrechartContext}
</prechart_context_background>

<current_dictation_authoritative>
${text}
</current_dictation_authoritative>`
                : `Please review this dictated medical note and identify any issues${rewriteFullNote ? ', then provide both a complete rewritten version AND a separate A&P-only rewrite' : ''}:
...
${text}`;
            ...
            console.log(`[${new Date().toISOString()}] GPT-5.2 apGaps: ${result.apGaps?.length || 0} gaps found, apGapCheck: ${apGapCheck}, rewriteFullNote: ${rewriteFullNote}`);
            ...
            result.context = {
                ...
                prechartUsed: hasPrechartContext,
                encounterType: hasPrechartContext ? normalizedEncounterType : null,
                prechartContextSource,
                prechartContextEnabled: usePrechartContextEnabled,
                prechartBlendModeEnabled,
                prechartRoomCode: normalizedRoomCode || null,
                profileUsed: !!effectiveUserProfile,
                profileSource,
                reconcileRequested,
                reconcileApplied,
                validateRunId
            };
            result.reconcileDebug = {
                baselineChars: hasPrechartContext ? safePrechartContext.length : 0,
                todayUpdateChars: text.length,
                mode: reconcileApplied ? "baseline_plus_today_reconcile" : (hasPrechartContext ? "context_assist" : "dictation_only"),
                sourcePage: sourcePage || 'index',
                prechartSource: prechartContextSource,
                prechartEnabled: usePrechartContextEnabled,
                prechartBlend: prechartBlendModeEnabled,
                profileUsed: !!effectiveUserProfile,
                profileSource,
            ...
            console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} complete (gpt) mode=${result.reconcileDebug.mode} issues=${result.issues?.length || 0} apGaps=${result.apGaps?.length || 0}`);
    ...
    // ============ CLAUDE PATH ============
    // Select Claude model based on request (updated Feb 2026 to latest 4.6 models)
    const modelMap = {
        'sonnet': 'claude-sonnet-4-6',
        'opus': 'claude-opus-4-6'
    };
    ...
    // Per-model max output token limits (API hard ceilings)
    // Set to ~80% of ceiling to stay safe while giving plenty of room
    const modelMaxOutputMap = {
        'claude-opus-4-6': 102000,      // ceiling: 128,000
        'claude-sonnet-4-6': 51000,     // ceiling: 64,000
    };
    const baseMaxOutput = modelMaxOutputMap[selectedModel] || 51000;
...
`;
    ...
    console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} start (claude) model=${selectedModel} depth=${depth || 'quick'} thinking=${thinkingBudget} suggestions=${includeSuggestions || false} examData=${examData ? 'yes' : 'no'} profile=${effectiveUserProfile ? 'yes' : 'no'} profileSource=${profileSource} prechart=${hasPrechartContext ? 'yes' : 'no'} prechartSource=${prechartContextSource} prechartEnabled=${usePrechartContextEnabled ? 'yes' : 'no'} prechartBlend=${prechartBlendModeEnabled ? 'yes' : 'no'} room=${normalizedRoomCode || 'n/a'} reconcileRequested=${reconcileRequested ? 'yes' : 'no'} reconcileApplied=${reconcileApplied ? 'yes' : 'no'} debug=${debugRequested ? 'yes' : 'no'} encounter=${normalizedEncounterType} source=${sourcePage || 'index'} textChars=${text.length} contextChars=${safePrechartContext.length}`);
...
`;
    ...
    let prechartContextInstructionBlock = '';
    const prechartBlendInstructionBlock = prechartBlendModeEnabled ? prechartBlendModeInstruction : '';
    const prechart2DigestBlockInstructionBlock = prechart2DigestBlockInstruction;
    const prechart2CurrentTruthInstructionBlock = prechart2CurrentTruthInstruction;
    const advancedReasoningInstructionBlock = advancedReasoningInstruction;
    let prechartPayloadBlock = `Dictation to review:\n${text}`;
    if (hasPrechartContext) {
        prechartContextInstructionBlock = reconcileApplied
            ? `
## PRECHART RECONCILIATION MODE:
You are reconciling two sources into ONE current encounter note:
1) PreChart baseline (pre-visit background)
...
- Encounter mode is ${normalizedEncounterType}.`;
        ...
        prechartPayloadBlock = reconcileApplied
            ? `Reconcile baseline with today's update.
Do a deliberate baseline history sweep so no relevant prior history is lost.

Encounter type:
${normalizedEncounterType}

PreChart baseline (background):
${safePrechartContext}

Today's dictation update (authoritative):
${text}`
            : `Encounter type:
${normalizedEncounterType}

PreChart context (background only):
${safePrechartContext}

Current dictation to review (authoritative for today):
${text}`;
    }
    
    try {
        // max_tokens: use generous per-model budget, add thinking on top
        // baseMaxOutput is ~80% of model's hard ceiling
        const maxTokens = thinkingBudget > 0 ? thinkingBudget + baseMaxOutput : baseMaxOutput;
...
${ignoreRulesSection}
${examInsertionInstruction}
${prechartContextInstructionBlock}
${prechartBlendInstructionBlock}
${prechart2DigestBlockInstructionBlock}
${prechart2CurrentTruthInstructionBlock}
${advancedReasoningInstructionBlock}
## CHECKS TO PERFORM:
...
Return JSON only (no markdown, no backticks):
{
  ...
  "apGaps": ${apGapCheck && rewriteFullNote ? `[
    {
      "finding": "The specific clinical finding from HPI, exam, or diagnostics not addressed in A&P",
      "source": "HPI|Physical Exam|Diagnostics|ROS",
      "planItem": "A complete, ready-to-insert A&P entry with problem name, patient-specific clinical reasoning, and concrete evidence-based management plan"
    }
  ]` : '[]'},
  "summary": "Brief 1-line summary"
}
...
If no issues: {"issues": [], "icd10": [], "guidelineAlerts": [], "modifiedNote": null, "examInsertedAt": null, "suggestions": null, "rewrittenNote": null, "rewrittenAP": null, "originalAPStart": null, "originalAPEnd": null, "removedContent": [], "apGaps": [], "summary": "No issues found"}
${rewriteFullNote ? `
## REWRITE FULL NOTE:
You have been asked to provide a complete rewritten version of the note. In "rewrittenNote":
- Apply ALL corrections from the issues you identified
...
${prechartPayloadBlock}`
        ...
        // ============ SSE STREAMING MODE ============
        // When client requests SSE (stream: true), we pipe progress events to keep
        // Render's 100s proxy timeout from killing long Opus requests.
        // The final event contains the complete JSON result — same shape as before.
        const useSSE = clientWantsSSE && model !== 'gpt-5.2';
        let keepaliveInterval = null;
        
        if (useSSE) {
            // Set SSE headers — keeps Render proxy alive as long as data flows
            res.writeHead(200, {
            ...
            // Send keepalive pings every 5s so proxy sees activity
            const sseStart = Date.now();
            keepaliveInterval = setInterval(() => {
                try { res.write(`event: ping\ndata: {"elapsed":${((Date.now() - sseStart) / 1000).toFixed(1)}}\n\n`); } catch(e) {}
            }, 5000);
            
            // Send initial status
            res.write(`event: status\ndata: {"phase":"thinking","model":"${selectedModel}"}\n\n`);
        }
        
        // Stream from Anthropic SDK
        const stream = anthropic.messages.stream(apiOptions);
        
        // Track if we've seen the first text token (for SSE status updates)
        ...
        // Check if response was truncated (hit max_tokens ceiling)
        if (message.stop_reason === 'max_tokens') {
            console.log(`[${new Date().toISOString()}] ⚠️ Claude response TRUNCATED - max_tokens (${maxTokens}) reached. Model: ${selectedModel}`);
        }
        ...
        console.log(`[${new Date().toISOString()}] Claude apGaps: ${result.apGaps?.length || 0} gaps found, apGapCheck: ${apGapCheck}, rewriteFullNote: ${rewriteFullNote}`);
        
        // Add actual usage and cost info (matching GPT-5.2 response format)
        ...
        // Claude 4.6 pricing per 1M tokens
        const claudePricing = model === 'opus' 
            ? { input: 5.0, output: 25.0 }    // Opus 4.6
            : { input: 3.0, output: 15.0 };    // Sonnet 4.6
        
        const inputCost = (inputTokens / 1_000_000) * claudePricing.input;
        const outputCost = (outputTokens / 1_000_000) * claudePricing.output;
        const totalCost = inputCost + outputCost;
        
        console.log(`[${new Date().toISOString()}] Claude usage: ${inputTokens} in (${cacheHit} cached), ${outputTokens} out, cost: $${totalCost.toFixed(6)}`);
        
        result.usage = {
            inputTokens,
            outputTokens,
            reasoningTokens: 0,  // thinking tokens are included in outputTokens for Claude
        ...
        console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} complete (claude) mode=${result.reconcileDebug.mode} issues=${result.issues?.length || 0} apGaps=${result.apGaps?.length || 0}`);
        
        // Send result via SSE or regular JSON
        if (useSSE) {
        ...
        if (useSSE) {
            try {
                clearInterval(keepaliveInterval);
                res.write(`event: error\ndata: ${JSON.stringify({ error: 'Validation failed', message: err.message })}\n\n`);
                res.end();
            } catch(e) {
...
const COST_TABLE = {
    // Claude 4.6 pricing per 1M tokens (Feb 2026)
    'claude-opus': { input: 5.0, output: 25.0, name: 'Claude Opus 4.6' },
    'claude-sonnet': { input: 3.0, output: 15.0, name: 'Claude Sonnet 4.6' },
    'claude-haiku': { input: 0.80, output: 4.0, name: 'Claude Haiku 4.5' },
        ...
        extractPrompt: `Extract cath lab findings into structured JSON:
{
  ...
  "rawText": "Full text"
    ...
    lab_results: {
        ...
        extractPrompt: `Extract ALL lab values into structured JSON. IMPORTANT: If results span multiple dates/times, capture each date separately. Group results by collection date, with newest date first. For each panel, include the collection date and time:
{
  "documentType": "lab_results",
  "patientInfo": { "name": null, "mrn": null, "dos": null, "room": null },
  ...
  "panels": [
    {
      ...
      "results": [
        { "test": "test name", "value": "numeric or text value", "unit": "unit", "refRange": "reference range", "flag": "H|L|C|null", "priorValue": "value from previous date if visible" }
      ]
    }
  ],
  "trendingLabs": [
    {
      ...
      "values": [
        { "date": "YYYY-MM-DD", "time": "HH:MM", "value": "numeric", "flag": "H|L|C|null" }
      ]
    }
  ],
        ...
        extractPrompt: `Extract the clinical note into structured JSON:
{
  ...
  "rawText": "Full text"
    ...
    patient_list: {
        ...
        extractPrompt: `Extract the patient list into structured JSON. For each patient, summarize their key labs as a short string (e.g. "BNP 892↓, Cr 1.8↑, K 3.1↓") and key vitals as a short string (e.g. "BP 108/68, HR 82, O2 96%"). Assess acuity as "high", "medium", or "low" based on clinical status. Determine current status (e.g. "stable", "pre-cath", "improving", "post-procedure"):
{
  "documentType": "patient_list",
  "unitInfo": { "unit": null, "date": null, "attending": null },
  "patients": [
    {
      ...
      "keyLabs": "summary string with arrows for trends",
      "keyVitals": "summary string",
      "activeMeds": [],
      "briefHx": null
    }
  ],
...
// --- Helper: Call Claude for image analysis (uses streaming to avoid SDK timeout) ---
async function callClaudeWithImages(images, systemPrompt, userPrompt, model, settings = {}) {
    ...
    // Per-model output token limits (~80% of API ceiling):
    // claude-opus-4-6: 128,000 max → 102,000
    // claude-sonnet-4-6: 64,000 max → 51,000
    // claude-haiku-4-5-20251001: 64,000 max → 51,000
    const maxTokensMap = {
        'claude-opus': 102000,
        'claude-sonnet': 51000,
        'claude-haiku': 51000
    };
    const modelMaxTokens = maxTokensMap[model] || 51000;
    ...
    // max_tokens must cover both thinking + output but cannot exceed model's hard limit
    if (settings.thinkingBudget && settings.thinkingBudget > 0) {
        apiOptions.thinking = { type: "enabled", budget_tokens: Math.min(settings.thinkingBudget, modelMaxTokens - 4096) };
        apiOptions.max_tokens = modelMaxTokens;
    }
    
    // Use streaming to avoid SDK 10-minute timeout on large max_tokens
    // Collect all chunks server-side — iOS still gets a single JSON response
    let responseText = '';
    let usage = {};
    ...
    const regularInputTokens = inputTokens - cacheHit - cacheCreated;
    const cost = ((regularInputTokens / 1_000_000) * pricing.input) 
               + ((cacheHit / 1_000_000) * pricing.input * 0.1)
               + ((cacheCreated / 1_000_000) * pricing.input * 1.25)
               + ((outputTokens / 1_000_000) * pricing.output);
    
    console.log(`[AI] ${model} | ${(elapsed/1000).toFixed(1)}s | cache: ${cacheHit} read, ${cacheCreated} created | tokens: ${inputTokens}in/${outputTokens}out | stop: ${finalMessage.stop_reason}`);
    
    return {
        text: responseText,
        model: selectedModel,
        timing: { ms: elapsed, seconds: (elapsed / 1000).toFixed(1) },
        usage: { inputTokens, outputTokens, totalTokens: inputTokens + outputTokens, cacheHit, cacheCreated },
...
function parseAIJson(text) {
    ...
    try {
        ...
        // If truncated (model ran out of tokens), try to recover
        console.log(`[${new Date().toISOString()}] JSON parse failed, attempting truncation recovery...`);
        return recoverTruncatedJson(clean);
    }
}
...
function recoverTruncatedJson(text) {
    ...
    // Strip any trailing incomplete key-value or partial structure
    // Find the last complete value (ends with ", or number, or ], or }, or true/false/null)
    // First, try to cut back to last clean line
    const lines = fixed.split('\n');
    
    // Strategy 1: Close brackets approach
    for (let attempt = 0; attempt < 3; attempt++) {
        let tryText = fixed;
        
        if (attempt === 1) {
            // Remove last 1-3 potentially broken lines
            const trimLines = lines.slice(0, -2).join('\n');
            if (trimLines.length > 50) tryText = trimLines;
        }
        if (attempt === 2) {
            const trimLines = lines.slice(0, -5).join('\n');
            if (trimLines.length > 50) tryText = trimLines;
        }
        
        // Close any unclosed string
        const quoteCount = (tryText.match(/(?<!\\)"/g) || []).length;
        if (quoteCount % 2 !== 0) tryText += '"';
        
        // Remove trailing comma
        tryText = tryText.replace(/,\s*$/, '');
        
        // Count open brackets/braces
        let openBraces = 0, openBrackets = 0, inString = false;
        for (let i = 0; i < tryText.length; i++) {
            const c = tryText[i];
            if (c === '"' && (i === 0 || tryText[i-1] !== '\\')) { inString = !inString; continue; }
        ...
        for (let i = 0; i < openBrackets; i++) tryText += ']';
        for (let i = 0; i < openBraces; i++) tryText += '}';
        
        try {
            const result = JSON.parse(tryText);
            console.log(`[${new Date().toISOString()}] ✅ Truncation recovery SUCCESS (attempt ${attempt+1}) — salvaged ${Object.keys(result).length} top-level keys`);
            result._truncated = true;
            return result;
        } catch { /* try next attempt */ }
    }
    
    // Strategy 2: Find the last position where we have a valid complete object
    // Work backwards from the end to find the last }]} that balances
    const lastBrace = fixed.lastIndexOf('}');
    if (lastBrace > 100) {
        for (let end = lastBrace; end >= Math.max(100, lastBrace - 5000); end--) {
            if (fixed[end] !== '}' && fixed[end] !== ']') continue;
            const sub = fixed.substring(0, end + 1);
            let ob = 0, obrk = 0, inStr = false;
            for (let i = 0; i < sub.length; i++) {
                const ch = sub[i];
                if (ch === '"' && (i === 0 || sub[i-1] !== '\\')) { inStr = !inStr; continue; }
                if (inStr) continue;
                if (ch === '{') ob++; if (ch === '}') ob--;
                if (ch === '[') obrk++; if (ch === ']') obrk--;
            }
            if (ob === 0 && obrk === 0) {
                try {
                    const result = JSON.parse(sub);
                    console.log(`[${new Date().toISOString()}] ✅ Aggressive recovery SUCCESS — trimmed ${fixed.length - end} chars, got ${Object.keys(result).length} keys`);
                    result._truncated = true;
                    return result;
                } catch { continue; }
            }
    ...
    // Strategy 3: Just try to get panels/labs array even if outer object is broken
    const panelsMatch = fixed.match(/"panels"\s*:\s*(\[[\s\S]*)/);
    if (panelsMatch) {
        let panelsText = panelsMatch[1];
        // Try to close it
        const quoteCount = (panelsText.match(/(?<!\\)"/g) || []).length;
        if (quoteCount % 2 !== 0) panelsText += '"';
        panelsText = panelsText.replace(/,\s*$/, '');
        let ob = 0, obrk = 0, inStr = false;
        for (let i = 0; i < panelsText.length; i++) {
            const c = panelsText[i];
            if (c === '"' && (i === 0 || panelsText[i-1] !== '\\')) { inStr = !inStr; continue; }
            if (inStr) continue;
            if (c === '{') ob++; if (c === '}') ob--;
            if (c === '[') obrk++; if (c === ']') obrk--;
        }
        for (let i = 0; i < obrk; i++) panelsText += ']';
        for (let i = 0; i < ob; i++) panelsText += '}';
        try {
            const panels = JSON.parse(panelsText);
            console.log(`[${new Date().toISOString()}] ✅ Panels-only recovery — got ${panels.length} panels`);
            return { documentType: 'lab_results', panels, _truncated: true };
        } catch { /* fall through */ }
    }
    
    console.error(`[${new Date().toISOString()}] ❌ JSON recovery FAILED — returning raw text`);
    return { documentType: 'unknown', rawText: text, parseError: true, _truncated: true };
}
...
Format each smartFlag as: { "flag": "description", "category": "improvement|decline|trend|monitoring_gap|critical|medication_interaction", "priority": "info|warning|critical" }`;
        ...
        if (images && images.length > 0) {
            ...
            if (interpreterModel === 'gpt-5.2') {
                ...
                const startTime = Date.now();
                const modelMap = { 'claude-opus': 'claude-opus-4-6', 'claude-sonnet': 'claude-sonnet-4-6', 'claude-haiku': 'claude-haiku-4-5-20251001' };
                const selectedInterpModel = modelMap[interpreterModel] || modelMap['claude-sonnet'];
                const interpMaxTokensMap = { 'claude-opus': 102000, 'claude-sonnet': 51000, 'claude-haiku': 51000 };
                const interpMaxTokens = interpMaxTokensMap[interpreterModel] || 51000;
                const apiOpts = {
                    model: selectedInterpModel,
                    max_tokens: interpMaxTokens,
                    system: systemPrompt,
                    messages: [{ role: "user", content: userPrompt }]
                };
                if (interpreterSettings.thinkingBudget && interpreterSettings.thinkingBudget > 0) {
                    apiOpts.thinking = { type: "enabled", budget_tokens: Math.min(interpreterSettings.thinkingBudget, interpMaxTokens - 4096) };
                    apiOpts.max_tokens = interpMaxTokens;
                }
                const stream = anthropic.messages.stream(apiOpts);
...
// Health check
// ══════════════════════════════════════════════════════════
// ── DICTATION HISTORY ENDPOINTS ──
// ══════════════════════════════════════════════════════════

// Browse dictations by date for a room
// GET /api/dictations/:room/dates — list all dates that have dictations
app.get('/api/dictations/:room/dates', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT DATE(created_at AT TIME ZONE 'America/Chicago') as date,
                   COUNT(*) as count
            FROM dictations WHERE room = $1
...
// Get dictations for a specific date
// GET /api/dictations/:room/date/:date (date = YYYY-MM-DD)
app.get('/api/dictations/:room/date/:date', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT id, patient_name, preview,
                   created_at AT TIME ZONE 'America/Chicago' as created_at
            FROM dictations
            WHERE room = $1
              AND DATE(created_at AT TIME ZONE 'America/Chicago') = $2
            ORDER BY created_at DESC
        `, [req.params.room, req.params.date]);
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Search dictations — exact (ILIKE) or fuzzy (pg_trgm)
// GET /api/dictations/:room/search?q=keyword&mode=exact|fuzzy
// NOTE: This MUST be defined before /:room/:id or Express matches "search" as :id
app.get('/api/dictations/:room/search', async (req, res) => {
    const { q, mode = 'exact' } = req.query;
    if (!q || q.trim().length < 2) return res.status(400).json({ error: 'Query too short (min 2 chars)' });

    try {
        let result;
        const query = q.trim();
        if (mode === 'fuzzy') {
            // Use word_similarity + ILIKE fallback. Avoid the % operator which can crash the pg driver.
            result = await pool.query(`
                SELECT id, patient_name, preview,
                       created_at AT TIME ZONE 'America/Chicago' as created_at,
                       GREATEST(similarity(lower(text), lower($2)), word_similarity(lower($2), lower(text))) as score
                FROM dictations
                WHERE room = $1
                  AND (text ILIKE concat('%%', $2::text, '%%')
                       OR word_similarity(lower($2), lower(text)) > 0.15
                       OR similarity(lower(text), lower($2)) > 0.05)
                ORDER BY GREATEST(similarity(lower(text), lower($2)), word_similarity(lower($2), lower(text))) DESC, created_at DESC
                LIMIT 100
            `, [req.params.room, query]);
        } else {
...
// Get full text of a single dictation
app.get('/api/dictations/:room/:id', async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT * FROM dictations WHERE id = $1 AND room = $2',
            [req.params.id, req.params.room]
        );
...
// Update patient name manually
app.patch('/api/dictations/:room/:id', async (req, res) => {
    const { patient_name } = req.body;
    try {
        await pool.query(
            'UPDATE dictations SET patient_name = $1 WHERE id = $2 AND room = $3',
            [patient_name || '', req.params.id, req.params.room]
        );
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
// Delete a dictation
app.delete('/api/dictations/:room/:id', async (req, res) => {
    try {
        await pool.query('DELETE FROM dictations WHERE id = $1 AND room = $2', [req.params.id, req.params.room]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Serve dictation history page
app.get('/dictation-history', (req, res) => {
    res.sendFile(path.join(__dirname, 'dictation-history.html'));
});
