...
app.post('/api/validate', async (req, res) => {
    ...
    console.log(`[${new Date().toISOString()}] Validating ${text.length} chars`);
...
**Abbreviation Expansions**: List all medical abbreviations used and their full meanings (e.g., "EF" → "Ejection Fraction", "SOB" → "Shortness of Breath")

**ICD-10 Suggestions**: Based on the diagnoses and conditions mentioned, suggest relevant ICD-10 codes with descriptions

Return JSON only (no markdown, no backticks):
{
  "issues": [
    {"type": "clinical_value", "severity": "error", "text": "'Normal ejection fraction 15%' is contradictory. 15% is severely reduced (normal is 55-70%). Likely transcription error - did you mean 50% or 55%?"},
    {"type": "transcription", "severity": "warning", "text": "'dye-a-beat-ease' likely means 'diabetes'"}
  ],
  "corrected": "The fully corrected text with all fixes applied, or null if no corrections needed",
  "abbreviations": [
    {"abbrev": "EF", "full": "Ejection Fraction"},
    {"abbrev": "LVEF", "full": "Left Ventricular Ejection Fraction"},
...
If no issues: {"issues": [], "corrected": null, "abbreviations": [...], "icd10": [...], "summary": "No issues found"}
...
${text}`
        ...
        console.log(`[${new Date().toISOString()}] Validation complete: ${result.issues.length} issues found`);
        res.json(result);
