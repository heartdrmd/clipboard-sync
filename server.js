...
## EXAM INSERTION TASK:
The user has prepared physical exam findings using the Exam Builder. You must INSERT this exam text into the appropriate location in the note.

**Exam text to insert:**
${examData.examText}

**Instructions:**
1. Find the physical exam section in the note. Look for headers like:
   - "PHYSICAL EXAM:" or "PE:" or "Exam:" or "Physical Examination:"
   - "[EXAM]" or "[PE]" placeholder
   - "OBJECTIVE:" section (in SOAP notes)
   - Any similar exam-related header
2. Replace or insert the exam text in that location
3. If no exam section exists, insert after vital signs or at appropriate location
4. In your response, include "examInsertedAt" field with the text/header where you inserted it
5. The modified note should be returned in "modifiedNote" field with the exam text wrapped in markers: %%EXAM_INSERT_START%% and %%EXAM_INSERT_END%% so the UI can highlight it
...
Return JSON only (no markdown, no backticks):
{
  ...
  "modifiedNote": "If exam was inserted, this contains the full note with %%EXAM_INSERT_START%% and %%EXAM_INSERT_END%% markers around the inserted exam text. Otherwise null.",
  "examInsertedAt": "If exam was inserted, describes where (e.g., 'Replaced PHYSICAL EXAM: section'). Otherwise null.",
