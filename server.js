...
app.post('/api/image-analyze/read', async (req, res) => {
    ...
    try {
        ...
        if (mode === 'rounding') {
            // Rounding-specific extraction prompt
...
app.post('/api/image-analyze/interpret', async (req, res) => {
    const { extractedData, interpreterModel = 'claude-sonnet', interpreterSettings = {}, images, mode, patientContext } = req.body;
    
    if (!extractedData) {
        return res.status(400).json({ error: 'No extracted data provided' });
    }
    
    console.log(`[${new Date().toISOString()}] CardioScan interpret-only: model=${interpreterModel}, type=${extractedData.documentType}, mode=${mode || 'clinic'}`);
