...
app.post('/api/validate', async (req, res) => {
    ...
    console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} source=${sourcePage || 'index'} model=${model || 'sonnet'} prechartContext=${hasPrechartContext ? 'yes' : 'no'} prechartSource=${prechartContextSource} prechartEnabled=${usePrechartContextEnabled ? 'yes' : 'no'} room=${normalizedRoomCode || 'n/a'} reconcileRequested=${reconcileRequested ? 'yes' : 'no'} reconcileApplied=${reconcileApplied ? 'yes' : 'no'} encounter=${normalizedEncounterType} textChars=${text.length} contextChars=${safePrechartContext.length}`);
