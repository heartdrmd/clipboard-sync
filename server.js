...
async function callClaudeWithImages(images, systemPrompt, userPrompt, model, settings = {}) {
    ...
    const apiOptions = {
        model: selectedModel,
        // Haiku 4.5 supports 64K output; Sonnet/Opus 8K default. Use 16K for all — multi-date labs can be 30K+ chars
        max_tokens: 65536,
        // Use prompt caching on system prompt — same schema every call
        system: [
            {
                type: "text",
                text: systemPrompt,
    ...
    console.log(`[AI] ${model} | ${(elapsed/1000).toFixed(1)}s | cache: ${cacheHit} read, ${cacheCreated} created | tokens: ${inputTokens}in/${outputTokens}out`);
