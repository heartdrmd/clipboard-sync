...
app.post('/api/generate-attestation', async (req, res) => {
    ...
    try {
        const systemPrompt = `You are a medical documentation assistant helping a supervising cardiologist (Dr. Aker) create audit-proof attestation statements.

The physician personally conducts 80%+ of:
- Patient interview (history taking)
- Physical examination  
- Clinical evaluation and analysis
- Medical decision making and plan formulation

The NP/PA participates in discussions and documents the encounter as a scribe.

Your task: Extract key clinical elements from the note and create a personalized attestation that proves the physician understood and evaluated this specific patient. This is NOT boilerplate - it must contain case-specific details.

## OUTPUT FORMAT (follow exactly):

I, Dr. Aker, personally saw and evaluated this patient, performing >80% of the interview, examination, clinical evaluation, and medical decision-making. The APP participated in discussions and documented this encounter.

**Chief Concern:** [1 line - main reason for visit]

**Key History:** [2-3 bullet points - most important history elements]

**Pertinent Exam Findings:** [2-3 bullet points - significant exam findings]

**Clinical Reasoning:** [1-2 sentences showing you understood the case]

**Assessment:** [List the diagnoses/problems addressed]

**Plan Summary:** [2-4 bullet points - key management decisions made]

I have reviewed and agree with the documentation above.

---

## RULES:
- Extract ONLY what is in the note - never add or infer
- Keep it cliff-notes style - brief but specific
- Include specific values (BP, HR, EF%, medications, doses) when present
- Show medical decision-making (why tests ordered, why meds chosen)
- This must pass auditor scrutiny: "Can I tell the physician understood THIS patient?"
- No generic statements - every line should be patient-specific`;

        const userPrompt = `Generate an audit-proof attestation for this clinic note:
...
app.post('/api/validate', async (req, res) => {
    ...
    if (model === 'gpt-5.2') {
        ...
        try {
            ...
            let systemPrompt = `You are a meticulous medical documentation editor. Task: rewrite and format the provided clinician draft into a clean, professional cardiology follow-up note. Preserve ALL clinical meaning. Do NOT add new facts, diagnoses, meds, vitals, or test results. Fix grammar, spelling, terminology, and structure. Resolve ambiguous phrasing only by rewording—do not infer. Use standard headings: Reason for Visit, HPI, ROS, Physical Exam, Diagnostics, Assessment & Plan (numbered problems), Diagnoses (ICD-10 list). Keep concise but complete. Output only the edited note; no commentary.
...
${text}`;
            ...
            // Use temperature from request (default 0.2 for consistency)
            const apiTemperature = temperature || 0.2;
            
            // Call GPT-5.2 API with temperature for consistent output
            // Temperature 0.1-0.2 keeps output deterministic and avoids "creative" additions
            const response = await openai.responses.create({
                model: "gpt-5.2",
                input: [
                    { role: "system", content: systemPrompt },
