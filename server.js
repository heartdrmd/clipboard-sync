...
app.post('/api/validate', async (req, res) => {
    ...
    const hasPrechartContext = typeof prechartContext === 'string' && prechartContext.trim().length > 0;
    const normalizedEncounterType = encounterType === 'new_consult' ? 'new_consult' : 'follow_up';
    const safePrechartContext = hasPrechartContext ? prechartContext.trim().slice(0, 30000) : '';
    const isPrechartPage = sourcePage === 'prechart';
    const reconcileRequested = reconcileMode === true || reconcileMode === 'true' || reconcileMode === 1 || reconcileMode === '1';
    const reconcileApplied = hasPrechartContext && (reconcileRequested || isPrechartPage);
    const debugRequested = prechartDebug === true || prechartDebug === 'true' || prechartDebug === 1 || prechartDebug === '1';
    const validateRunId = `v_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    
    console.log(`[${new Date().toISOString()}] Validate run=${validateRunId} source=${sourcePage || 'index'} model=${model || 'sonnet'} prechartContext=${hasPrechartContext ? 'yes' : 'no'} reconcileRequested=${reconcileRequested ? 'yes' : 'no'} reconcileApplied=${reconcileApplied ? 'yes' : 'no'} encounter=${normalizedEncounterType} textChars=${text.length} contextChars=${safePrechartContext.length}`);
